  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data?.token) {
        ...
        toast.success('注册成功', { description: `欢迎加入 CampusSync，${name}！` });
        navigate('/', { replace: true });
      } else {
        setError(res.message || '注册失败，请检查信息后重试');
      }
    } catch {
      setError('网络错误，请稍后重试');
    } finally {
    ...
    <div className="min-h-screen bg-[oklch(0.955_0.008_240)] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[oklch(0.48_0.05_240)] text-sm">创建账户，开始智能管理您的学习日程</p>
        </div>

        {/* Card */}
        <div className="bg-white rounded-2xl shadow-[0_10px_24px_-3px_rgb(30_58_95_/_0.15)] p-8">
          <h2 className="text-xl font-bold text-[oklch(0.12_0.025_240)] mb-6 font-serif">创建账户</h2>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-[oklch(0.48_0.05_240)]">
              已有账户？{' '}
              <Link to="/login" className="font-semibold text-[oklch(0.28_0.07_240)] hover:underline">
                立即登录
              </Link>
            </p>
          </div>
